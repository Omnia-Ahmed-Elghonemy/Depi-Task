...
select * from production.products  where list_price >1000 --no--
select * from sales.customers where state='CA' or state='NY'
select * from sales.orders where required_date ='2023'--no--
select * from sales.customers where email like '%gmail.com'
select * from sales.staffs where ,        --مش عارفة القيمة تبقى اي بالظبط--

select top 5 product_name ,list_price from production.products order by list_price asc --no data--
select top 10 order_id ,order_date from sales.orders order by order_date  --no data--
 select top 3 first_name ,last_name from sales.customers order by last_name
 select * from sales.customers where phone is null
 select * from sales.staffs where manager_id is not null    --no data--
 select category_id, count(product_id) as total_products from production.products
 group by category_id   --no data--

 select state ,count(customer_id) as total_customers from sales.customers 
 group by state

 select brand_id,AVG(list_price) as avg_of_listprice from production.products
 group by brand_id     --no data--

 select staff_id ,COUNT(order_id) as total_orders from sales.orders
 group by staff_id   --no data--

 select customer_id ,COUNT(order_id) as total_orders_greater_than_2 from sales.orders
 group by customer_id having COUNT(order_id) >2     --no data--

 select * from production.products where list_price between 500 and 1000     --no data--

  select * from sales.customers where city like 's%'

  select * from sales.orders where order_status in(2,4)    --no data--

  select * from production.products where category_id in(1,2,3)     --no data--
   select * from sales.staffs where store_id=1 or phone is null     --no data--